...
// NEW: Keys for Strict Mode settings
...
const String prefAppBlocking = "appBlocking";
const String prefExitBlocking = "exitBlocking";

/// Enum cho các Strict Mode features
...
class HomeCubit extends Cubit<HomeState> {
  ...
  static const MethodChannel _notificationChannel = MethodChannel(
    'com.example.moji_todo/notification',
  );
  static const MethodChannel _serviceChannel = MethodChannel(
    'com.example.moji_todo/app_block_service',
  );
  static const MethodChannel _permissionChannel = MethodChannel(
    'com.example.moji_todo/permissions',
  );
  static const EventChannel _eventChannel = EventChannel(
    'com.example.moji_todo/timer_events',
  );
  ...
  void _initAudioPlayerListeners() {
    _playerStateSubscription = _audioPlayer.onPlayerStateChanged.listen((
      ...
      if (s == PlayerState.completed) {
        print(
          '[AudioPlayer] Playback completed. Current state: isWhiteNoiseEnabled=${state.isWhiteNoiseEnabled}, selectedWhiteNoise=${state.selectedWhiteNoise}, isTimerRunning=${state.isTimerRunning}, isPaused=${state.isPaused}',
        );
        ...
        if (_canPlayWhiteNoise && state.isTimerRunning && !state.isPaused) {
          print(
            '[AudioPlayer] Attempting to replay white noise due to completion.',
          );
        ...
        print(
          '[AudioPlayer] Playback stopped. Current state: isWhiteNoiseEnabled=${state.isWhiteNoiseEnabled}, selectedWhiteNoise=${state.selectedWhiteNoise}, isTimerRunning=${state.isTimerRunning}, isPaused=${state.isPaused}',
        );
      } else if (s == PlayerState.paused) {
        print(
          '[AudioPlayer] Playback paused. Current state: isWhiteNoiseEnabled=${state.isWhiteNoiseEnabled}, selectedWhiteNoise=${state.selectedWhiteNoise}, isTimerRunning=${state.isTimerRunning}, isPaused=${state.isPaused}',
        );
      }
    });
  ...
  Future<void> _initialize() async {
    ...
    final isWhiteNoiseEnabled =
        prefs.getBool(prefWhiteNoiseEnabled) ?? false; //
    final selectedWhiteNoise =
        prefs.getString(prefSelectedWhiteNoise) ?? 'none'; //
    ...
    final notificationSound =
        prefs.getString(prefNotificationSound) ?? 'bell'; //
    final autoSwitch = prefs.getBool(prefAutoSwitch) ?? false;

    // Load Strict Mode settings from SharedPreferences
    final blockNotifications = prefs.getBool(prefBlockNotifications) ?? false;
    final blockPhoneCalls = prefs.getBool(prefBlockPhoneCalls) ?? false;
    final lockPhone = prefs.getBool(prefLockPhone) ?? false;
    final appBlocking = prefs.getBool(prefAppBlocking) ?? false;
    final exitBlocking = prefs.getBool(prefExitBlocking) ?? false;

    // Calculate isStrictModeEnabled based on loaded settings
    final isStrictModeEnabled =
        blockNotifications ||
        blockPhoneCalls ||
        appBlocking ||
        lockPhone ||
    ...
    emit(
      state.copyWith(
        ...
        isBlockNotificationsEnabled: blockNotifications,
        isBlockPhoneCallsEnabled: blockPhoneCalls,
        isLockPhoneEnabled: lockPhone,
        isAppBlockingEnabled: appBlocking,
        isExitBlockingEnabled: exitBlocking,
    ...
    _timerSubscription = _eventChannel.receiveBroadcastStream().listen(
      (event) async {
        ...
        final isCountingUp =
            args['isCountingUp'] as bool? ?? state.isCountingUp;
        final isWorkSessionFromService =
            args['isWorkSession'] as bool? ?? state.isWorkSession;

        print(
          'Timer update received: timerSeconds=$timerSeconds, isRunning=$isRunning, isPaused=$isPaused, isCountingUp=$isCountingUp, isWorkSession=$isWorkSessionFromService',
        );
          ...
          if (state.autoSwitch) {
            if (isWorkSessionFromService) {
              ...
              if (state.currentSession < state.totalSessions) {
                ...
                await _notificationChannel.invokeMethod(TimerActions.stop);
                print(
                  'Fall-through in auto-switch: all sessions completed or logic error.',
                );
              }
            }
          } else {
            ...
            if (state.selectedTask != null) {
              await _updateTaskPomodoroState(
                state.selectedTask,
                false,
                state.timerSeconds,
              );
            ...
            await _notificationChannel.invokeMethod(TimerActions.stop);
            print(
              'Session completed. Auto-switch is off. Timer service stopped.',
            );
          }
    ...
    _notificationChannel.setMethodCallHandler((call) async {
      switch (call.method) {
        case 'onSessionEnded':
          ...
          print(
            'Received onSessionEnded from Native (app in background): isWorkSession=$isWorkSession',
          );
        ...
        case 'completedAllSessions':
          print(
            'Handling completedAllSessions from notification (via MainActivity).',
          );
          emit(
            state.copyWith(
              timerSeconds: state.workDuration * 60,
        ...
        case 'pause':
          print(
            'Handling pause from notification via MethodChannel. Flutter UI should update via EventChannel.',
          );
          return null;
        case 'resume':
          print(
            'Handling resume from notification via MethodChannel. Flutter UI should update via EventChannel.',
          );
          return null;
        case 'stop':
          print(
            'Handling stop from notification via MethodChannel. Flutter UI should update via EventChannel.',
          );
          return null;
        case 'showEndSessionNotification':
          print(
            'showEndSessionNotification received via MethodChannel (might be deprecated)',
          );
          return null;
  ...
  Future<void> _handleSessionEnded(bool isWorkSessionThatEnded) async {
    print(
      '_handleSessionEnded called for session type: ${isWorkSessionThatEnded ? "Work" : "Break"}',
    );

    await _notificationService.cancelNotification(
      id: UnifiedNotificationService.TIMER_NOTIFICATION_ID,
    );
    ...
    if (!_hasShownEndSessionNotification) {
      try {
        ...
        print(
          'Showed end session notification from _handleSessionEnded with soundEnabled: ${state.soundEnabled}, sound: ${state.notificationSound}',
        );
      } catch (e) {
        print(
          'Error showing end session notification from _handleSessionEnded: $e',
        );
      }
    }
  }
  ...
  void selectTask(String? taskTitle, int estimatedPomodoros) {
    emit(
      state.copyWith(
        ...
        totalSessions:
            taskTitle != null ? estimatedPomodoros : state.totalSessions,
        currentSession: 0,
        timerSeconds: state.workDuration * 60,
        isTimerRunning: false,
        isPaused: false,
  ...
  void startTimer() {
    ...
    emit(
      state.copyWith(
        ...
        isPaused: false,
        currentSession:
            isWorkSessionToStart && !state.isTimerRunning && !state.isPaused
  ...
  void pauseTimer() {
    ...
    emit(state.copyWith(isTimerRunning: false, isPaused: true));
    if (state.selectedTask != null) {
    ...
    if (_audioPlayer.state == PlayerState.playing) {
      // NEW: Check player state before pausing
      _audioPlayer.pause();
  ...
  void continueTimer() {
    ...
    emit(state.copyWith(isTimerRunning: true, isPaused: false));
  ...
  void stopTimer({bool updateTaskState = true}) {
    ...
    // Disable strict mode settings when timer stops
    _disableStrictModeSettings();
    _updateAppBlockingState();

    _updateSharedPreferences(
      state.isCountingUp ? 0 : state.workDuration * 60,
      false,
      false,
      true,
    );
    _audioPlayer.stop();
    print(
      '[AudioPlayer] Called stop() explicitly in stopTimer.',
    ); // NEW: Log explicit stop
    _hasPlayedSoundOnEnd = false;
    _hasShownEndSessionNotification = false;
    _endSessionNotificationTimer?.cancel();
  }
  ...
  void updateStrictMode({
    ...
    final newBlockNotificationsEnabled =
        isBlockNotificationsEnabled ?? state.isBlockNotificationsEnabled;
    final newBlockPhoneCallsEnabled =
        isBlockPhoneCallsEnabled ?? state.isBlockPhoneCallsEnabled;
    final newAppBlockingEnabled =
        isAppBlockingEnabled ?? state.isAppBlockingEnabled;
    final newFlipPhoneEnabled = isFlipPhoneEnabled ?? state.isFlipPhoneEnabled;
    final newLockPhoneEnabled = isLockPhoneEnabled ?? state.isLockPhoneEnabled;
    final newExitBlockingEnabled =
        isExitBlockingEnabled ?? state.isExitBlockingEnabled;

    final isStrictModeEnabled =
        newBlockNotificationsEnabled ||
        newBlockPhoneCallsEnabled ||
        newAppBlockingEnabled ||
        newFlipPhoneEnabled ||
        newLockPhoneEnabled ||
        newExitBlockingEnabled;

    emit(
      state.copyWith(
        isStrictModeEnabled: isStrictModeEnabled,
        isBlockNotificationsEnabled: newBlockNotificationsEnabled,
        isBlockPhoneCallsEnabled: newBlockPhoneCallsEnabled,
        isAppBlockingEnabled: newAppBlockingEnabled,
        isFlipPhoneEnabled: newFlipPhoneEnabled,
        isLockPhoneEnabled: newLockPhoneEnabled,
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(prefBlockNotifications, newBlockNotificationsEnabled);
    await prefs.setBool(prefBlockPhoneCalls, newBlockPhoneCallsEnabled);
    await prefs.setBool(prefLockPhone, newLockPhoneEnabled);
    await prefs.setBool(prefAppBlocking, newAppBlockingEnabled);
    await prefs.setBool(prefExitBlocking, newExitBlockingEnabled);
    print('Saved Strict Mode settings to SharedPreferences');

    _updateAppBlockingState();
    _applyStrictModeSettings();

    if (newExitBlockingEnabled && _canPlayWhiteNoise && state.isTimerRunning) {
      if (_audioPlayer.state == PlayerState.playing) {
        _audioPlayer.pause();
        print(
          '[AudioPlayer] Called pause() due to Strict Mode (Exit Blocking).',
        );
      }
    } else if (!newExitBlockingEnabled &&
        _canPlayWhiteNoise &&
        state.isTimerRunning &&
        !state.isPaused) {
      _playWhiteNoise(state.selectedWhiteNoise!);
  ...
  /// Check permission WITHOUT requesting (just check current state)
  /// Returns true if permission is already granted or not needed
  bool checkPermissionOnly(StrictModeFeature feature) {
    switch (feature) {
      case StrictModeFeature.blockNotifications:
        return state.hasDNDPermission;
  ...
  Future<bool> checkAndRequestPermissionFor(StrictModeFeature feature) async {
    switch (feature) {
      case StrictModeFeature.blockNotifications:
        ...
        // Note: Usually user needs to grant it in Settings, so this will still be false
  ...
  Future<void> refreshPermissionStates() async {
    ...
    print('Refreshed permissions: DND=$hasDND, Accessibility=$hasAccessibility');
  ...
  /// Disable all strict mode settings (called when timer stops)
  void _disableStrictModeSettings() {
    _serviceChannel.invokeMethod('setKeepScreenOn', {'enabled': false});
    _serviceChannel.invokeMethod('setBlockNotifications', {'enabled': false});
    print('Disabled all strict mode settings');
  }
  ...
  Future<void> updateTimerMode({
    ...
    emit(
      state.copyWith(
        ...
        timerSeconds:
            state.isTimerRunning ? state.timerSeconds : (newWorkDuration * 60),
        isWorkSession: true,
        isCountingUp: isCountingUp,
      ),
    );
    ...
    print(
      'Saved sound settings to SharedPreferences: soundEnabled=$soundEnabled, notificationSound=$notificationSound, autoSwitch=$newAutoSwitch',
    );
  ...
  void restoreTimerState({
    ...
    print(
      '[HomeCubit] restoreTimerState called: isRunning=$isRunning, isPaused=$isPaused, isWhiteNoiseEnabled=${state.isWhiteNoiseEnabled}, selectedWhiteNoise=${state.selectedWhiteNoise}',
    );
    emit(
      state.copyWith(
        timerSeconds: timerSeconds,
    ...
    if (isRunning && !isPaused && _canPlayWhiteNoise) {
      ...
      _audioPlayer.stop();
      print(
        '[AudioPlayer] Called stop() in restoreTimerState (conditions not met or timer not running/paused).',
      ); // NEW: Log
    }
  }
  ...
  void toggleWhiteNoise(bool enable) async {
    print(
      '[HomeCubit] toggleWhiteNoise called with enable: $enable. Current selected: ${state.selectedWhiteNoise}',
    );
    ...
    if (enable &&
      ...
      _audioPlayer.stop();
      print(
        '[AudioPlayer] Called stop() in toggleWhiteNoise (enable is false or conditions not met).',
      ); // NEW: Log
    }
  }
  ...
  void selectWhiteNoise(String sound) async {
    ...
    if (sound != 'none' &&
      ...
      _audioPlayer.stop();
      print(
        '[AudioPlayer] Called stop() in selectWhiteNoise (sound is none or conditions not met).',
      ); // NEW: Log
    }
  }
  ...
  Future<void> _playWhiteNoise(String sound) async {
    print(
      '[HomeCubit] _playWhiteNoise called with sound: $sound. Current state: isTimerRunning=${state.isTimerRunning}, isPaused=${state.isPaused}',
    );
    if (sound == 'none' || sound.isEmpty) {
      print(
        '[AudioPlayer] _playWhiteNoise: Sound is none or empty, stopping player.',
      );
    ...
    try {
      ...
      print(
        '[AudioPlayer] Volume set to ${state.whiteNoiseVolume} in _playWhiteNoise.',
      );
      ...
      if (state.isTimerRunning && !state.isPaused) {
        ...
        print(
          '[AudioPlayer] Conditions not met to play white noise (timer not running or paused). Player remains stopped/paused.',
        );
      }
    } catch (e) {
      ...
      if (kDebugMode) {
        print(
          '[AudioPlayer] White noise sound file not found or error playing: $sound.mp3. Exception: ${e.toString()}',
        );
      }
      // Cân nhắc việc emit một trạng thái lỗi ở đây nếu cần
      emit(
        state.copyWith(isWhiteNoiseEnabled: false, selectedWhiteNoise: 'none'),
      ); // Tự động tắt white noise nếu có lỗi
    }
  }

  Future<void> _startTimerServiceCall(
    int seconds,
    bool isRunning,
    bool isPaused,
    bool isCountingUp,
  ...
  Future<void> _updateTaskPomodoroState(
    String? taskTitle,
    bool isActive,
    ...
    final taskKey = taskBox.keys.firstWhere((k) {
  ...
  Future<void> _playSound() async {
    // This is for notification sound, not white noise.
    ...
    print(
      '[AudioPlayer] Attempting to play notification sound (in-app): $soundFile with _audioPlayer instance.',
    );
    ...
    try {
      ...
      print(
        '[AudioPlayer] Successfully played sound (in-app): $soundFile --- THIS LOGIC SHOULD BE REVIEWED IF _playSound IS STILL USED.',
      );
    } catch (e) {
      print('[AudioPlayer] Error playing sound (in-app): $e');
      if (kDebugMode) {
        print(
          '[AudioPlayer] Notification sound file not found or error playing (in-app): $soundFile',
        );
      }
    }
  }
  ...
  Future<void> _updateSharedPreferences(
    ...
    print(
      'Updated SharedPreferences: timerSeconds=$timerSeconds, isRunning=$isRunning, isPaused=$isPaused, isCountingUp=${state.isCountingUp}, isWorkSession=$isWorkSession',
    );
  }
  ...
  Future<void> close() {
    ...
    _playerStateSubscription
        ?.cancel(); // NEW: Hủy subscription của player state
